...
class GameScreenObj3D extends StatefulWidget {
  const GameScreenObj3D({super.key});

  @override
  State<GameScreenObj3D> createState() => _GameScreenObj3DState();
}

class _GameScreenObj3DState extends State<GameScreenObj3D> with TickerProviderStateMixin {
            ...
            child: Container(
              ...
              child: Cube(
                onSceneCreated: (Scene scene) {
                  ...
                  // Загружаем 3D модель стула
                  try {
                    _stool = Object(
                      fileName: 'assets/models/character/cnek.obj',
                      scale: math.Vector3(0.1, 0.1, 0.1), // Масштабируем модель
                    );
                    scene.world.add(_stool!);
                  } catch (e) {
                    print('Ошибка загрузки 3D модели: $e');
                    // Если не удалось загрузить, создаем простой куб
                    _stool = Object(
                      mesh: Mesh.cube(),
                  ...
                  // Настраиваем камеру
                  scene.camera.position = math.Vector3(0, 2, 5);
  ...
  void _update3DObjects() {
    if (_scene == null || _stool == null) return;
    
    final gameProvider = Provider.of<GameProvider>(context, listen: false);
    final playerZ = gameProvider.gameState.distance;
    
    // Обновляем позицию стула
    _playerX = gameProvider.gameState.currentLane * 2.0 - 2.0;
    _stool!.position = math.Vector3(_playerX, 0, 0);
    
    // Обновляем камеру (следует за стулом)
    _scene!.camera.position = math.Vector3(_playerX, 2, 5);
